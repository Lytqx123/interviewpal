...
import {
  createSession, startInterview, nextQuestion, buildBaselinePlan, closeInterview, getSessionSummary, ingestSignal,
...
describe('面试官 · 引擎（规则路径）', () => {
  ...
  it('nextQuestion：3 轮追问，第 3 轮 shouldClose=true', async () => {
    const session = createSession({ resumeProfile: RESUME, jobProfile: jobProfile('tech'), maxDepth: 3 });
    await startInterview(session);
    const f1 = await nextQuestion(session, '我负责订单系统');
    assert.equal(f1.shouldClose, false);
    assert.equal(session.depth, 1);
    const f2 = await nextQuestion(session, '用了 Redis 做缓存');
    assert.equal(f2.shouldClose, false);
    assert.equal(session.depth, 2);
    const f3 = await nextQuestion(session, 'Redis 是单线程模型');
    assert.equal(f3.shouldClose, true);
    assert.equal(session.depth, 3);
    assert.equal(session.phase, 'closing');
  });

  it('nextQuestion：追问方向随深度递进', async () => {
    const session = createSession({ resumeProfile: RESUME, jobProfile: jobProfile('tech') });
    await startInterview(session);
    const areas = [];
    for (let i = 0; i < 3; i++) {
      const f = await nextQuestion(session, `候选人回答第${i + 1}轮`);
      areas.push(f.focusArea);
    }
    assert.deepEqual(areas, ['关键经历 STAR 展开', '实现细节与决策理由', '极端场景与对比方案']);
  });

  it('nextQuestion：maxDepth 可配置', async () => {
    const session = createSession({ resumeProfile: RESUME, jobProfile: jobProfile(), maxDepth: 2 });
    await startInterview(session);
    const f1 = await nextQuestion(session, '回答1');
    assert.equal(f1.shouldClose, false);
    const f2 = await nextQuestion(session, '回答2');
  ...
  it('对话历史：面试官与候选人交替记录', async () => {
    const session = createSession({ resumeProfile: RESUME, jobProfile: jobProfile() });
    await startInterview(session);
    await nextQuestion(session, '回答1');
    await nextQuestion(session, '回答2');
...
describe('面试官 · LLM 路径', () => {
  ...
  it('nextQuestion：LLM 路径解析追问', async () => {
    const session = createSession({ resumeProfile: RESUME, jobProfile: jobProfile(), llm: fakeLlm() });
    await startInterview(session);
    const f = await nextQuestion(session, '我做过订单系统');
  ...
  it('LLM 失败时落回规则兜底', async () => {
    ...
    const f = await nextQuestion(session, '回答');
...
describe('面试官 · 预分析策略模式（方案书 §5.4）', () => {
  ...
  it('createSession：不传 preanalysisPlan 显式回退规则模式，传了走策略模式', () => {
    ...
    const s = createSession({ resumeProfile: RESUME, jobProfile: jobProfile(), preanalysisPlan: plan });
    assert.equal(s.mode, 'preanalysis');
  ...
  it('nextQuestion：卡顿信号触发降档追问（深→中）', async () => {
    const session = createSession({ resumeProfile: RESUME, jobProfile: jobProfile('tech'), preanalysisPlan: plan });
    await startInterview(session);
  ...
  it('executionTrace：关闭后输出每题耗时、信号、是否换线', async () => {
    ...
    assert.equal(summary.mode, 'preanalysis');
  ...
  it('失忆不变量：两场 session 独立，信号与 baseline 不串扰', async () => {
    const a = createSession({ resumeProfile: RESUME, jobProfile: jobProfile('tech'), preanalysisPlan: plan });
    const b = createSession({ resumeProfile: RESUME, jobProfile: jobProfile('tech'), preanalysisPlan: plan });
  ...
  it('nextQuestion：preanalysis 模式 LLM 可用时保留规则决策的调整标记', async () => {
